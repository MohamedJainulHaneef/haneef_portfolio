...
function Skills() {
    return (
        <section className="w-full min-h-screen flex items-center justify-center bg-gray-50" id="skills">
            <motion.div className="w-full max-w-7xl mx-auto p-6 sm:p-8 md:p-8 bg-white rounded-2xl shadow-xl border border-teal-100" variants={sectionContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                ...
                <div className="space-y-8">
                    {categorizedSkills.map(category => (
                        <div key={category.category}>
                            <motion.h3 className="flex items-center gap-4 text-2xl lg:text-3xl font-bold text-gray-800 mb-6 md:mb-8 pb-3 md:pb-5 border-b-2 border-teal-100"
                                variants={categoryTitle} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.5 }}>
                                <span className="text-3xl">{category.icon}</span>
                                {category.category}
                            </motion.h3>

                            <motion.div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8"
