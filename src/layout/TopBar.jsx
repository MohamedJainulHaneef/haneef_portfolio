...
function useResponsiveOffset() {
    ...
    useEffect(() => {
        const calculateOffset = () => {
            const newOffset = window.innerWidth < 768 ? -350 : -70;
            setOffset(newOffset);
        }
        calculateOffset();
...
function TopBar({ menuVisible, setMenuVisible }) {
    ...
    return (
        <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/90 shadow-lg transition-all border-b border-gray-100">
            <div className="flex items-center justify-between max-w-7xl mx-auto px-4 py-3.5 lg:px-6 2xl:px-16">
                    ...
                    {menus.map((menu) => (
                        <ScrollLink
                            ...
                            className="relative cursor-pointer font-semibold text-lg text-gray-700 hover:text-teal-600 transition-all duration-200
                                after:absolute after:left-0 after:bottom-0 after:h-0.5 after:bg-teal-600 after:transition-all after:duration-300 after:ease-in-out after:w-0 hover:after:w-full 2xl:text-xl"
                        >
                            {menu.name}
                        </ScrollLink>
                    ))}
            ...
            {/* Mobile Dropdown Menu - LIGHT THEME IMPLEMENTED HERE */}
            <div
                className={`md:hidden transition-all duration-500 ease-in-out transform overflow-hidden
                    ${menuVisible ? 'max-h-screen opacity-100 translate-y-0' : 'max-h-0 opacity-0 -translate-y-4 pointer-events-none'}`
                }
            >
                <div className="bg-white shadow-xl border-t border-gray-100 px-4 py-3 space-y-2 rounded-b-lg">
                    {menus.map((menu) => (
                        <ScrollLink
                            key={menu.name}
                            to={menu.path}
                            ...
                            className="block text-lg font-semibold text-gray-800 px-4 py-2 rounded-lg hover:bg-teal-50 hover:text-teal-600 transition-all duration-200"
                            activeClass="text-teal-600 font-bold bg-teal-50"
                        >
                            {menu.name}
                        </ScrollLink>
                    ))}
